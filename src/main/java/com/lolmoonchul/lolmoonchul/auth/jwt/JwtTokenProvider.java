package com.lolmoonchul.lolmoonchul.auth.jwt;

import com.lolmoonchul.lolmoonchul.auth.exception.AuthenticationException;
import com.lolmoonchul.lolmoonchul.member.domain.vo.Role;
...
public class JwtTokenProvider {
    ...
    public Role extractRoleFromAccessToken(final String token) {
        validateAccessToken(token);
        final Jws<Claims> claimsJws = getAccessTokenParser().parseClaimsJws(token);
        return Role.valueOf(claimsJws.getBody().get(ROLE_KEY, String.class));
    }
    ...
    public Role extractRoleFromRefreshToken(final String token) {
        validateAccessToken(token);
        final Jws<Claims> claimsJws = getRefreshTokenParser().parseClaimsJws(token);
        return Role.valueOf(claimsJws.getBody().get(ROLE_KEY, String.class));
    }
